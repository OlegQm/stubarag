        ...
        if prompt := st.chat_input(
            ...
            with st.chat_message("assistant", avatar=st.session_state.assistant_icon):
                ...
                try:
                    ...
                    st.error(f"An error occurred while contacting the assistant: {e}")
                    response = st.session_state.translator("Sorry, something went wrong. Please try again later.")
            ...
            if (not st.session_state.historical) and ("rec_id" not in st.session_state):
                try:
                    ...
                    st.error(f"Could not open conversation history: {e}")
            ...
            try:
                if st.session_state.rec_id is not None:
                    history.record_history(
                        st.session_state.messages, st.session_state.rec_id)
            except Exception as e:
                st.error(f"Could not record conversation history: {e}")
