...
def pytest_terminal_summary(terminalreporter, exitstatus, config):
    ...
    """
    failed_tests = len([result for result in results if "invalid" in next(iter(result.values()))])
    ...
    for i, result in enumerate(results):
        terminalreporter.write_line(f"{i + 1}) Question: " + str(next(iter(result.keys()))))
